...
class ContactController(base.BaseController):
    ...
    def _submit(context):
        ...
        if len(errors) == 0:
            ...
            body += '\n\nReason: %s\n\nSent by:\nName:%s\nEmail: %s\n' % (data_dict['razon'],data_dict["name"], data_dict["email"])
            mail_dict = {
                'recipient_email': config.get("ckanext.panama.mail_to", config.get('email_to')),
                'recipient_name': config.get("ckanext.panama.recipient_name", config.get('ckan.site_title')),
                'subject': config.get("ckanext.panama.subject", 'Question from visitor'),
                'body': body,
